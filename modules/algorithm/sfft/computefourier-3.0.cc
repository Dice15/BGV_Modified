...
#else
#pragma message("Compiling without Intel Performance Primitives - resulting code will be slower!")
...
{
    complex_t shift = std::polar(1.0, 2 * M_PI * offset * b / n);
    complex_t step = std::polar(1.0, 2 * M_PI * ai * b / n);
    ...
    int W_Man, int offset)
{
    ...
    double value_real = real(value);
    double value_imag = imag(value);
            ...
            if (zero_buck_check[j] > ZERO_BUCK_CHECK_CUTOFF)
            {
                    ...
                    && (c0 > 0.01))
                {
                    ...
                    if (hashed_to == i + j)
                    {
                        ...
                        median_value = complex_t(median_phase_cos, median_phase_sin);
                        median_value =
                            c0 * median_value;
                        FLOPCOUNT_INCREMENT(2);
    ...
    complex_t* filterf2)
{
    ...
    thread_local static std::uniform_int_distribution<> rand(0, n - 1);
    thread_local static std::uniform_real_distribution<> rand48(0.0, 1.0);
    ...
    int init_offset = (unsigned)floor(rand48(gen) * n);
    int init_G_offset = (unsigned)floor(rand48(gen) * n);
    ...
    while (true)
    {
        ...
        if (nana % 3 == 2)
        {
            ...
            {
                if (cabs2(MAN_SAMP[2 * j] / static_cast<double>(W_Man)) > 1e-6)
                    count_man++;
                FLOPCOUNT_INCREMENT(2 + 1);
            }
