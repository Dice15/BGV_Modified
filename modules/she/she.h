    ...
    */
    ...
    public:
        ...
        */
        ...
        /**
        Retrieves the encryption scheme currently in use.

        @details
        The function assigns one of the following values to the destination parameter based on the scheme:
        - "bfv": Brakerski/Fan-Vercauteren (BFV) scheme for integer arithmetic.
        - "ckks": Cheon-Kim-Kim-Song (CKKS) scheme for approximate arithmetic.
        - "bgv": Brakerski-Gentry-Vaikuntanathan (BGV) scheme for integer arithmetic.

        @param[out] destination A reference to a string that will be overwritten with the scheme name.
        */
        void scheme(std::string& destination) const;

        /**
        Retrieves the encryption scheme currently in use as a string.

        @details
        The function internally calls the overloaded `scheme(std::string&)` method
        to retrieve the scheme name. It returns the following values based on the scheme:
        - "bfv": Brakerski/Fan-Vercauteren (BFV) scheme for integer arithmetic.
        - "ckks": Cheon-Kim-Kim-Song (CKKS) scheme for approximate arithmetic.
        - "bgv": Brakerski-Gentry-Vaikuntanathan (BGV) scheme for integer arithmetic.

        @return A string containing the name of the encryption scheme in use.
        */
        std::string scheme() const;

        /**
        Retrieves the number of slots available in the ciphertext.

        @details
        - For the BGV and BFV schemes, the number of slots is equal to half the polynomial modulus degree.
        - For the CKKS scheme, the number of slots corresponds to half the polynomial modulus degree, allowing complex numbers to be stored.
        The function determines the scheme type and fetches the slot count using the appropriate encoder.

        @param[out] destination A reference to a variable that will hold the number of slots.
        */
        void slot_count(size_t& destination) const;

        /**
        Returns the number of slots available in the ciphertext.

        @details
        This function simplifies slot retrieval by internally calling the version that outputs the result to a reference.
        ...
        */
        ...
        @details
        This function fetches the plain modulus value from the encryption parameters.
        It is only supported for BGV and BFV schemes. Throws an exception for unsupported schemes.

        @param[out] destination A reference to a variable that will store the plain modulus value.
        ...
        */
        ...
        /**
         ...
         @details
         This function internally calls `plain_modulus_prime(uint64_t&)` to fetch the plain modulus value.

         @return The plain modulus value.
